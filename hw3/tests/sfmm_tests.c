...
//############################################
// 
Test(sf_memsuite_student, realloc_equal_block_free_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(double) * 8);
	void *y = sf_realloc(x, sizeof(double)*8);

	cr_assert_not_null(y, "y is NULL!");
	// sf_header *hp = (sf_header *)((char*)y - sizeof(sf_footer));
	sf_header *hp = (sf_header *)((char*)y - sizeof(sf_footer));
	cr_assert(hp->info.allocated == 1, "Allocated bit is not set!");
	cr_assert(hp->info.block_size << 4 == 80, "Realloc'ed block size not what was expected!");
	cr_assert(hp->info.requested_size == 64, "Requested size not what was expected!");
	// After realloc'ing x, we can return a block of size 48 to the freelist.
	// This block will coalesce with the block of size 3968.
	assert_free_block_count(1);
	assert_free_list_count(3968, 1);
}
Test(sf_memsuite_student, realloc_muchbigger_block_free_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int) * 8);
	void *y = sf_realloc(x, sizeof(double)*100);

	cr_assert_not_null(y, "y is NULL!");
	// sf_header *hp = (sf_header *)((char*)y - sizeof(sf_footer));
	sf_header *hp = (sf_header *)((char*)y - sizeof(sf_footer));
	cr_assert(hp->info.allocated == 1, "Allocated bit is not set!");
	cr_assert(hp->info.block_size << 4 == 816, "Realloc'ed block size not what was expected!");
	cr_assert(hp->info.requested_size == 800, "Requested size not what was expected!");
	// After realloc'ing x, we can return a block of size 48 to the freelist.
	// This block will coalesce with the block of size 3968.
	assert_free_block_count(2);
	assert_free_list_count(48, 1);
}
Test(sf_memsuite_student, malloc_zero_pages, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(1);
...
Test(sf_memsuite_student, realloc_twotime_block, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int));
	/* void *y = */ sf_malloc(10);
	x = sf_realloc(x, sizeof(int) * 10);
	x = sf_realloc(x, sizeof(double)*100);
...
Test(sf_memsuite_student, freenull, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(sizeof(int));
	sf_header *hp = (sf_header *)((char *)x);
	sf_free(hp);

	cr_assert(hp->info.allocated == 0, "Allocated bit is not set!");
	assert_free_block_count(1);
	cr_assert(sf_errno == 0, "sf_errno is not 0!");
}